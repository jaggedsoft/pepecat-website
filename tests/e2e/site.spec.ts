...
test('publishes complete Open Graph and Twitter social metadata', async ({ page, request }) => {
  ...
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute('content', 'https://pepecat-nine.vercel.app/media/hero/pepecat-hero-cat-extracted.png?v=1')
  ...
  await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content', 'https://pepecat-nine.vercel.app/media/hero/pepecat-hero-cat-extracted.png?v=1')
  await expect(page.locator('meta[name="twitter:image:alt"]')).toHaveAttribute('content', /PEPECAT mascot/)

  const card = await request.get('/media/hero/pepecat-hero-cat-extracted.png')
...
test('renders the editorial page structure and working anchor navigation', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText(/the cat has entered the chat/i)
  await expect(page.locator('.hero__stage img')).toHaveAttribute('src', '/media/avatars/pc03-960.webp')
  await expect(page.locator('img[src*="/media/avatars/"]')).toHaveCount(1)
