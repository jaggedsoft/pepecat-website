...
test('publishes complete Open Graph and Twitter social metadata', async ({ page, request }) => {
  ...
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute('content', 'https://pepecat.vip/social-card-pepecat-v2.png')
  ...
  await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content', 'https://pepecat.vip/social-card-pepecat-v2.png')
  await expect(page.locator('meta[name="twitter:image:alt"]')).toHaveAttribute('content', /PEPECAT mascot/)

  const card = await request.get('/social-card-pepecat-v2.png')
